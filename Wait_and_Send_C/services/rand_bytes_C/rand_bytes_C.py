...
os.system("clear")
print 'Random Data is Receiving from ', dst_addr
...
while 1:
    try:
        ...
        payload_recv_array.insert(n,payload_recv[:10])
        payload_recv_array = payload_recv_array[-n:]
        crc_recv_array.insert(n,crc_recv)
        crc_recv_array = crc_recv_array[-n:]
        with open('/home/pi/Desktop/Wait_and_Send_C/services/rand_bytes_C/packets_recv_frm_AlltoC.csv','w') as f:
        ...
        if msg != '':
            if dst_addr_recv == src_addr:
                if packet_type_recv == packet_type[4]:
                    ...
                    if crc == crc_recv:
                        os.system("clear")
                        print 'Random Bytes Received. Printing...'
                        print 'Packet Type is       : ', packet_type_recv
                        print 'Source Node is       : ', src_addr_recv
                        print 'Real Destination is  : ', real_dst_addr_recv
                        print 'Sequence No is       : ', seq_num_recv
                        print 'Payload is           : ', payload_recv
                        print 'CRC is               : ', crc_recv
                        print 'Packet Length        : ', str(packet_length)
                        print '\n', 'Sending ACK...', '\n'

                        with open('/home/pi/Desktop/Wait_and_Send_C/services/rand_bytes_C/packets_recv_frm_AtoC_Only.csv','w') as f:
                            writer = csv.writer(f,delimiter='\t');
                            writer.writerows(zip(time_recv_array,packet_type_recv_array,src_addr_recv_array,dst_addr_recv_array,real_dst_addr_recv_array,seq_num_recv_array,payload_recv_array,crc_recv_array))
