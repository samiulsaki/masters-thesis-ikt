...
def CONNECTION_on(dst_addr):
    acknowledge = str(packet_type[4]+src_addr+dst_addr+real_dst_addr)#+mock_seq_num+mock_payload+mock_crc)
    return acknowledge
...
real_dst_addr = "0B"
input = CONNECTION_on(real_dst_addr)# packet_type[4] (rand_bytes) with conn. on
ser.write(input)
t.sleep(1.5)

os.system("clear")
print 'Random Data is Sending to ', real_dst_addr
input = 1
exitflag =0
...
while 1:
    try:
        ...
        with open('/home/pi/Desktop/Wait_and_Send_A/services/rand_bytes_B/packets_recv_frm_BtoA.csv','w') as f:
        ...
        if packet_type_recv == packet_type[2]:
            ...
            if ack_recv == '111':
                ...
                payload = bytes((random.randint(1111,9999))) + format(0,'0155d')
                ...
                packet_type_sent_array.insert(n,packet_type[4])
                packet_type_sent_array = packet_type_sent_array[-n:]
                time_sent_array.insert(n,time_sent)


                with open('/home/pi/Desktop/Wait_and_Send_A/services/rand_bytes_B/packets_sent_frm_AforB.csv','w') as f:
                                writer = csv.writer(f,delimiter='\t');
                ...
                t.sleep(1)
                input = str(packet_type[4]+src_addr+dst_addr+real_dst_addr+seq_num+payload+crc)
                ser.write(input)
                i = i+1
                pass
            pass
            ...
            if seq_num_recv in seq_num_array:
                ...
                input = str(packet_type[4]+src_addr+dst_addr+real_dst_addr+seq_num_recv+payload_array[v]+crc_retransmit)
                ser.write(input)
                t.sleep(1)
