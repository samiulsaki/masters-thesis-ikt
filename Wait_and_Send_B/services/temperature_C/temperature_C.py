...
os.system("clear")
print 'Temperature is Receiving from ', dst_addr_A
print 'Temperature is Forwarding to ', dst_addr_C
...
while 1:
    try:
        ...
        if packet_type_recv == packet_type[7]:
            if src_addr_recv == dst_addr_A:
                if dst_addr_recv == src_addr:
                    ...
                    print 'Temperature Received from ', src_addr_recv
                    print 'Forwarding Packets to ', dst_addr_C

                    input = str(packet_type[7]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+payload_recv+mock_payload_recv+crc_recv)
                    ser.write(input)
            ...
            if src_addr_recv == dst_addr_C:
                if dst_addr_recv == src_addr:
                    if seq_num_recv in seq_num_recv_array:
                        ...
                        input = str(packet_type[7]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+payload_recv_array[r]+mock_payload_recv+crc_recv_array[r])
                        ser.write(input)
        ...
        execfile('/home/pi/Desktop/Wait_and_Send_B/main.py')
                    ...
                    # input = str(packet_type[4]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+)


                    # print 'Sending ACK now to', src_addr_recv,'\n'
                    # input = str(packet_type[2]+ src_addr+dst_addr_A+real_dst_addr+seq_num_recv+mock_payload+mock_crc)
                    # ser.write(input)
                    # print input
                    # t.sleep(0.5)
...
#                     print 'Transaction in Progress...'
#                     print 'ACK Received from ', src_addr_recv
#                     print 'Forwarding Packets to', src_addr_recv,'\n'
#                     total_packet_send = 0
#                     seq_num_sent = str(format(i,'04d'))

#                     if seq_num_sent in seq_num_recv_array:
#                         # t.sleep(0.005)
#                         v = seq_num_recv_array.index(seq_num_sent)
# ##                        seq_num_sent = seq_num_recv_array[v]
#                         payload_sent = payload_recv_array[v]
#                         crc_sent = crc_recv_array[v]

#                         input = str(packet_type[4]+src_addr+dst_addr_C+real_dst_addr+seq_num_sent+payload_sent+crc_sent)
#                         ser.write(input)
#                         print(input)
#                         t.sleep(0.5)
#                         seq_num_sent_array.insert(n, seq_num)
#                         seq_num_sent_array = seq_num_sent_array[-n:]
#                         payload_sent_array.insert(n, payload_sent)
#                         payload_sent_array = payload_sent_array[-n:]
#                         crc_sent_array.insert(n, crc_sent)
#                         crc_sent_array = crc_sent_array[-n:]
#                         list_sent_C = zip(seq_num_sent_array, payload_sent_array, crc_sent_array)
...
#             if src_addr_recv == dst_addr_C:
#                 if dst_addr_recv == src_addr:
#                     if seq_num_recv in seq_num_sent_array:
#                         os.system("clear")
#                         t.sleep(0.005)
#                         r = seq_num_sent_array.index(seq_num_recv)
#                         print 'NACK Received from ',src_addr_recv, '. Retransmitting Packets to ', src_addr_recv
#                         input = str(packet_type[4]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+payload_sent_array[r]+crc_sent_array[r])
#                         ser.write(input)
#                         print(input)
#                         t.sleep(0.5)
#                         total_packet_send = total_packet_send + 1
