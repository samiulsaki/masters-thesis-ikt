...
os.system("clear")
print 'GPS Data is Receiving from ', dst_addr_A
print 'GPS Data is Forwarding to ', dst_addr_C
        ...
        if packet_type_recv == packet_type[6]:
            if src_addr_recv == dst_addr_A:
                if dst_addr_recv == src_addr:
                    ...
                    print 'GPS Data Received from ', src_addr_recv
                    print 'Forwarding Packets to ', dst_addr_C


                    input = str(packet_type[6]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+payload_recv+mock_payload_recv+crc_recv)
                    ser.write(input)
            ...
            if src_addr_recv == dst_addr_C:
                if dst_addr_recv == src_addr:
                    if seq_num_recv in seq_num_recv_array:
                        ...
                        input = str(packet_type[6]+src_addr+dst_addr_C+real_dst_addr+seq_num_recv+payload_recv_array[r]+mock_payload_recv+crc_recv_array[r])
                        ser.write(input)
